...
const handleSignIn = async () => {
  ...
  try {
    ...
    toast.add({ icon: CheckIcon, iconClass: "w-6 h-6 text-green-600", text: "Logged in", timeout: 6000 });
    router.push("/");
  } catch (err) {
    toast.add({
  ...
  <div class="w-full max-w-2xl mx-auto py-32 sm:pt-[13rem] px-3">
    <div>
      ...
      <form @submit.prevent="handleSignIn()">
        ...
        <div class="my-10">
          <Button size="sm" class="w-full" type="submit" :loading="isLoading"> Sign in </Button>
        </div>

        <p class="text-slate-700 text-center">Are you a new user? <AppLink to="/register">Sign up</AppLink></p>
      </form>
    </div>
  </div>
</template>
