    ...
    <form novalidate @submit.prevent="handleSignIn('email')">
      <div class="bg-white sn:px-20 sm:p-10 p-6 py-16 rounded-xl pb-14">
        <div class="mb-10 text-center">
          ...
          <p class="mt-2 text-slate-600">Log in to get the ability to comment, add reactions and more!</p>
        </div>

        <div class="flex flex-col gap-y-6">
          <Button type="button" size="sm" color="text-white bg-gray-700 hover:bg-gray-800 rounded-lg" @click="handleSignIn('github')">
        ...
        <div class="mt-5">
          ...
          <Input id="email" v-model="state.email" placeholder="Enter an email..." size="lg" type="email" class="mt-2" />
          <span> </span>
        </div>

        <div class="mt-5">
          <label for="password" class="text-slate-800"> Password </label>
          <Input id="password" v-model="state.password" placeholder="Enter a password..." size="lg" type="password" class="mt-2" />
        </div>

        <div class="my-10">
          <Button size="sm" class="w-full rounded-lg" type="submit" :loading="isLoading"> Submit </Button>
        </div>
      </div>
    </form>
  </div>
